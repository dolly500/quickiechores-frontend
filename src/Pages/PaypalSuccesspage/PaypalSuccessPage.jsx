...
const PayPalSuccessPage = () => {
  ...
  const pollingInterval = 5000; // 3 seconds between polling attempts
    ...
    const verifyPayment = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // If payment is pending, continue polling
          pollingTimeout = setTimeout(verifyPayment, pollingInterval);
        } else {
          // If payment failed (not pending), show error
        ...
        // On network or server error, continue polling to avoid premature failure
        pollingTimeout = setTimeout(verifyPayment, pollingInterval);
      }
    };
      ...
      <PageContainer>
        <Container>
          <ErrorContainer>
            ...
            <p>{error}</p>
            <ActionButtons>
              <PrimaryButton onClick={() => navigate('/book')}>
                Try Booking Again
              </PrimaryButton>
              <HomeButton onClick={() => navigate('/')}>
                <Home size={16} /> Go Home
